...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        self.generate_dataloader = DataLoader(dataset=generateSet, batch_size=1, shuffle=False, num_workers=opt.num_workers)
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.finetune_dataloader):
                ...
                for i in range(aspect_input_ids.shape[0]):
                    ...
                    start = int(torch.sum(inputs[2][i]).item())
...
def main():
    ...
    model_paths = {
        't5': resolve_model_path(opt.model_root, opt.t5_path, model_classes['t5']),
        'gpt': resolve_model_path(opt.model_root, '', model_classes['gpt']),
    }
    opt.model_class = model_paths[opt.model_name]
