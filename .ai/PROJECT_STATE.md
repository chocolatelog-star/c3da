...
2026年9月16日前完成并提交ICASSP 2027常规论文：4页英文技术正文，可选第5页仅用于参考文献、资助和伦理声明。届时必须完成最终方法冻结、正式实验、必要多随机种子、消融、BGCA（双向生成跨域方法）等基线对比、图表、引用核验、IEEE模板和PDF提交检查。方法冻结日期为9月8日，结果冻结日期为9月12日，9月15日完成提交演练。

实验主要在RTX 4090约24 GB显存服务器执行，默认并行2个独立实验；只有单任务峰值不超过约7 GB且三任务仍保留至少3 GB显存时才并行3个。没有可运行任务且代码工作预计超过30分钟时，应切换无GPU模式或关机。
...
当前元素感知工作树已提交结果卡聚合与入口审计修复`7e39287`；组件与句法图相关回归共75项通过。提交尚未推送，仍不得删除或启动正式GPU（图形处理器）实验。
