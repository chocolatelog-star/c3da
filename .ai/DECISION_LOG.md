...
## 2026-08-30：批准元素感知多三元组 RGAT 工程入口，正式训练仍未批准

- 研究变量严格限定为单一标量显著性头、消息源显著性注意力偏置、平衡元素聚焦损失和多元素覆盖损失；不新增图层、注意力头、MLP（多层感知机）或目标域金标监督。
- 历史图流程默认行为必须保持；新模块关闭时不保留额外注意力张量，避免改变历史显存与耗时。
- 服务器GPU（图形处理器）不可用时必须输出阻塞报告，不得用CPU（中央处理器）或降低精度冒充完整工程验收。
- GPU恢复后只允许运行入口审计，结果返回Chat Sol（研究负责人）后再决定是否批准快速消融。
