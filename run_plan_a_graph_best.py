...
def build_full_command(*, project_root: Path, adapter: Path, output_root: Path, model_path: str, train_batch_size: int, accumulation: int, cuda: str, seed: int, source_dataset: str = "rest16", target_dataset: str = "laptop14", final_lambda_domain_adv: float = 0.03) -> list[str]:
...
def main() -> int:
    ...
    p.add_argument("--recipe", default="configs/recipes/rest16_to_laptop14_m1_syntactic_rgat_pseudo_quick_ablation_v1.json")
    ...
    p.add_argument("--target_dataset", default="laptop14")
    ...
    full_cmd = build_full_command(project_root=root, adapter=adapter, output_root=Path(args.full_output_root), model_path=args.model_path, train_batch_size=args.train_batch_size, accumulation=args.gradient_accumulation_steps, cuda=args.cuda, seed=args.seed, source_dataset=args.source_dataset, target_dataset=args.target_dataset, final_lambda_domain_adv=0.0 if args.no_dann else 0.03)
