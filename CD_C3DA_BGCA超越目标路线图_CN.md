...
第五项代码已经完成，等待运行最终 5 轮单变量实验。实现从 T5 编码器原始句子上下文中提取方面词和观点词跨度，只使用源域人工标注多三元组句子；正确关系构成多正例，同句其他关系构成困难负例，并同时计算方面词到观点词、观点词到方面词两个方向。共享方面词或共享观点词不会被误当负例。当前最佳 1426 条训练数据中，352/352 个源域多三元组行全部成功定位，共 877 个有效配对，覆盖率 100%。

第五项固定当前最佳训练文件、DANN（领域对抗神经网络）0.03、情感对比学习 0.01、5 轮和全部解码参数，只新增系数 0.01、温度 0.1 的源域配对损失。训练日志新增配对损失、双向准确率和有效计数；评估新增单三元组/多三元组子集指标。主验收仍为 raw F1 是否超过 46.82，同时要求多三元组召回或配对错误获得改善。之后再研究跨度或配对级条件领域对抗和分层错误分析。
