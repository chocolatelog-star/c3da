...
当前进入第三项。原始目标伪标签只有 2 个中性预测，隐藏金标分析显示二者实际均为负向，所以不能降低中性筛选阈值或设置强制配额。改为固定 hp1 数据，只把中性样本的主生成损失提高：
...
49 个中性训练行的实际平均主生成权重为 1.6408、最大 2.0；1377 个非中性行最大仍为 1.0。代码版本为 `0c49ba6`，模型和指标统一使用 `neutral_gain100_max200` 标签。评估同时记录正向、负向、中性分项指标和中性否定误判。后续顺序保持为：生成器显式互相掩码训练、方面词-观点词配对损失、跨度或配对级条件领域对抗、分层错误分析。
