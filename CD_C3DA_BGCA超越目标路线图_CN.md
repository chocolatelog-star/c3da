...
GloVe（全局词向量）极性轴增强的 raw F1=44.70，低于旧最佳 46.63。只在最终模型加入监督式原型对比学习的第二版得到 raw F1=46.82、fixed F1=48.94，目前保留为最佳候选，但中性原型准确率最终为 0%，且相对提升只有 0.19。

全流程编码器对比实验已经完成：新提取器伪标签 F1 从 50.35 提高到 50.93，但最终 raw F1 降至 43.74。消融进一步得到：新上游无最终对比 raw F1=42.20；旧数据加编码器对比 raw F1=45.37；完整 T5 情感向量流程 raw F1=40.82。结论是伪标签总体 F1 的微小提高不能保证最终训练价值，纯情感聚类也不能替代三元组边界和配对学习。

编码器三元组覆盖分类头实验也已完成：raw F1=44.37、fixed F1=46.72。覆盖头长期把全部有效样本预测为单三元组，2 个和 3 个以上类别准确率为 0；测试集少生成 119 行、多生成仅 14 行。该辅助分类目标没有直接控制解码器输出数量，模型已删除，不继续提高覆盖损失权重。

完整双三元组监督已经完成：在原 421 条 hp1 伪标签上增加 73 条过滤前后均完整保留两个三元组的样本，明确排除 31 条被裁剪成单三元组的样本；新增样本权重为 0.25。组合伪标签共 494 行，隐藏金标 raw F1 从 50.35 提高到 51.08，最终训练集共 1499 行。D 组骨架关闭情感对比得到 raw P=55.45、raw R=42.33、raw F1=48.01、fixed F1=50.37，多三元组 raw R=35.57、F1=44.80。重新加入最终阶段源域平衡情感对比后得到 raw P=58.31、raw R=42.14、raw F1=48.93、fixed F1=50.21，多三元组 raw R=35.85、F1=46.13，成为当前新最佳。

严格模块消融 A-E 也已完成：A=42.32、B=44.75、C=41.42、D=46.63、E=46.82。增强单独贡献 +2.44 raw F1；DANN 单独贡献 -0.90，但在增强存在时贡献 +1.88，说明领域对抗必须依赖高质量目标风格增强；旧 hp1 数据上的情感对比只贡献 +0.19。完整双三元组加入后，情感对比进一步把 raw F1 从 48.01 提高到 48.93，并把多三元组 F1 从 44.80 提高到 46.13，说明结构覆盖改善后情感约束才开始稳定发挥作用。

改进路线第二项已经完成：把高精度伪标签每句最多三元组从 1 调整为 2 后，raw F1 从 46.82 降至 44.44。新增 104 行中，73 条完整双三元组质量尚可，但 31 条经距离过滤后只剩单三元组的样本 F1 只有 39.56；新增三元组还明显偏向正向。该实验未提高召回率，因此不进入最佳流程。
...
第三项已经完成。原始目标伪标签只有 2 个中性预测，隐藏金标分析显示二者实际均为负向，因此实验固定 hp1 数据，只提高中性样本的主生成损失：
...
49 个中性训练行的实际平均主生成权重为 1.6408、最大 2.0；1377 个非中性行最大仍为 1.0，代码确认增权真实生效。结果 raw F1=43.18、fixed F1=45.76，中性 F1 仍为 0；正向 raw F1 从 49.24 降至 45.06，负向从 52.86 降至 50.46。模型只新增 2 个中性预测，但均未完整匹配：一个情感和观点错误，另一个把两个中性三元组合并成错误边界。结论是中性问题主要来自目标域表示、边界和方面-观点配对，而不是损失权重不足。

`hp2` 与中性增权版失败模型、检查点、大型增强/训练数据和专属预测已删除，释放约 12.52 GB；总体指标、分情感指标、错误分析和伪标签分析保留。

第四项代码已经完成，等待运行 `rest16 -> laptop14` 单方向实验。新实验通过 `--reuse_upstream_run_dir`（复用上游实验目录）只读复用旧最佳实验的同一个提取器检查点、原始伪标签和 hp1 高精度伪标签，避免重新训练提取器带来随机误差。新生成器从 `J:\nlp\models\t5-base-py` 全新训练 8 轮，不复用旧生成器；训练数据按 `label_to_text`（标签到文本）、`masked_aspect`（方面词掩码）、`masked_opinion_sentiment`（观点词-情感掩码）组成 1:1:1 混合任务，真实训练集为 857:857:857，验证集为 210:210:210。增强阶段仍使用 `masked_mutual`（互相掩码），最终阶段继续使用 hp1 高精度伪标签、150 条权重 0.20 的增强、系数 0.03 的 DANN（领域对抗）以及系数 0.01 的源域类别平衡对比学习。因此该实验严格只检验“生成器训练与增强输入对齐”这一变量。

第四项已完成，raw F1=44.07、fixed F1=46.06，低于当前最佳 2.75。严格上游复用确认伪标签仍为同一批 421 条且 F1=50.35，因此下降来自混合生成器增强。混合生成器把一致性过滤通过数从 880 提高到 1200、模型回抽保留数从 273 提高到 546，但 NLI 保留率从 88.0% 降至 75.9%；最终 150 条增强中的多三元组行从 63 降至 23，三元组总数从 229 降至 177，方面词通道占比升至 86%。这说明局部标签一致性形成了确认偏差，没有补足全局结构和多三元组配对能力。失败生成器和最终模型已删除，释放约 11.65 GB，指标与分析保留。

第五项已经完成。实现从 T5 编码器原始句子上下文中提取方面词和观点词跨度，只使用源域人工标注多三元组句子；正确关系构成多正例，同句其他关系构成困难负例，并同时计算方面词到观点词、观点词到方面词两个方向。共享方面词或共享观点词不会被误当负例。当前最佳 1426 条训练数据中，352/352 个源域多三元组行全部成功定位，共 877 个有效配对，覆盖率 100%。

第五项固定当前最佳训练文件、DANN（领域对抗神经网络）0.03、情感对比学习 0.01、5 轮和全部解码参数，只新增系数 0.01、温度 0.1 的源域配对损失。结果 raw P=55.99、raw R=39.74、raw F1=46.49、fixed F1=48.86。与当前最佳相比，错误三元组减少 13 个，但正确三元组减少 6 个；单三元组 raw F1 从 52.85 提高到 53.16，多三元组 raw F1 从 42.65 降至 41.83，多三元组精确率从 59.20 提高到 60.64，召回率从 33.33 降至 31.93。结论是配对约束能减少错误组合，却让模型更保守，不能单独解决多三元组漏抽。失败模型已删除，释放约 5.83 GB，结构指标和代码历史保留。

下一阶段不继续提高配对损失权重。优先方向调整为：在标签到文本生成器、文本式领域前缀、hp1 高精度伪标签、150 条严格掩码增强、DANN 0.03 和最终阶段源域平衡情感对比 0.01 的最佳骨架上，引入显式三元组覆盖目标或解码阶段召回机制；中性问题则优先补充可信目标域中性样本和边界监督，不再单独提高类别损失权重。

### 已完成历史实验：全流程编码器情感原型对比学习

该实验的对比损失只使用 source gold（源域人工标注），完全排除伪标签和增强数据；857 句中的 1393 个观点跨度全部成功定位，覆盖率 100%，其中 pos=1015、neg=328、neu=50，平方根倒频率类别权重约为 0.413/0.726/1.861。初始提取器和最终模型的对比权重均为 0.01。
...
该历史实验使用独立目录 `runs\bgca_aste_stage1_full_contrastive_encoder_v1` 从 prepare（准备数据）开始完整重跑，结果已经完成并纳入顶部消融结论。

完整流程最终 raw F1=43.74、fixed F1=45.70，明显低于基线；新提取器高精度伪标签 F1=50.93，高于旧值 50.35。后续两组消融也已完成：A（新训练数据、关闭最终对比）raw F1=42.20；B（旧训练数据、开启编码器对比）raw F1=45.37。
...
第二轮结果：正向均值 +0.1575，负向均值 -0.1216；正向阈值 0.005817，负向阈值 -0.024779，中性绝对值带宽 0.177709；观点替换请求由 633 降至 493。候选还必须满足原观点相似度至少 0.35，无方面-观点共现时至少 0.50。模型回抽过滤进一步要求提取观点与请求观点的 GloVe（全局词向量）相似度至少 0.45，且极性一致。

完整单组实验得到 `raw P=52.36`、`raw R=39.00`、`raw F1=44.70`、`fixed F1=46.82`，低于当前最佳 46.63/48.98。它比上一版 T5 情感向量的 raw F1=43.13 提高 1.57，但仍存在方面词-观点词搭配不自然的问题，说明词向量极性和原词相似度不足以判断完整上下文兼容性。对应完整增强输出和最终模型已删除，仅保留小型诊断文件与 Git（版本管理）代码历史。下一步应研究上下文条件搭配评分，而不是继续单独微调词向量阈值。
