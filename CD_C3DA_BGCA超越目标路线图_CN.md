...
完整流程最终 raw F1=43.74、fixed F1=45.70，明显低于基线；但新提取器高精度伪标签 F1=50.93，高于旧值 50.35，且最终编码器原型中性准确率达到 50%。为分离上游数据变化和最终编码器约束，下一步固定随机种子 1000 跑两组消融：A 使用新训练数据但关闭最终对比学习；B 使用旧基线训练数据但开启最终编码器上下文对比学习。A/B 均保持 T5-base、5 轮、DANN=0.03、无约束解码和其余训练参数一致。
