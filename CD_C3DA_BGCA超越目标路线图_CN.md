...
当前从改进路线第二项开始，先做严格单变量实验：把高精度伪标签每句最多三元组从 1 调整为 2，距离上限仍为 5，其他参数全部固定。代码版本为 `6f2dcd3`，新输出统一使用 `hp2_dist5` 标签。该实验复用旧提取器、生成器和原始目标预测，只重新执行伪标签筛选、掩码增强、最终训练和评估。

```text
--high_precision_max_triplets 2
--high_precision_max_token_distance 5
pseudo_variants\hp2_dist5
```

后续顺序保持为：中性类别独立过滤与平衡、生成器显式互相掩码训练、方面词-观点词配对损失、跨度或配对级条件领域对抗、分层错误分析。每一步单独做消融，低于 raw F1=46.82 时先分析并询问是否删除。
