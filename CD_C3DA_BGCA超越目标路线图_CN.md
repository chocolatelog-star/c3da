...
第四项代码已经完成，等待运行 `rest16 -> laptop14` 单方向实验。新生成器从 `J:\nlp\models\t5-base-py` 全新训练 8 轮，不复用旧生成器；训练数据按 `label_to_text`（标签到文本）、`masked_aspect`（方面词掩码）、`masked_opinion_sentiment`（观点词-情感掩码）组成 1:1:1 混合任务，真实训练集为 857:857:857，验证集为 210:210:210。增强阶段仍使用 `masked_mutual`（互相掩码），最终阶段继续使用 hp1 高精度伪标签、150 条权重 0.20 的增强、系数 0.03 的 DANN（领域对抗）以及系数 0.01 的源域类别平衡对比学习。因此该实验只检验“生成器训练与增强输入对齐”这一变量。
