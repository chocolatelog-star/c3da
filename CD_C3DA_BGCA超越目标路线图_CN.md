...
下一项改为直接补充完整双三元组监督。数据构建已验证：在原 421 条 hp1 伪标签上增加 73 条过滤前后均完整保留两个三元组的样本，明确排除 31 条被裁剪成单三元组的样本；新增样本权重为 0.25。组合伪标签共 494 行，隐藏金标 raw F1 从 50.35 提高到 51.08，最终训练集共 1499 行。训练与评估尚待执行，验收重点是总体 raw F1 不低于 46.82 且多三元组召回高于 33.33%。

同时已建立严格模块消融 A-E：A 为源域+hp1，B 增加 150 条严格增强，C 在 A 上增加 DANN，D 同时使用增强和 DANN，E 再加入最终阶段源域平衡情感对比并对应当前最佳。四组新训练全部使用相同 5 轮、随机种子和解码参数，支持检查点续跑，完成后自动生成中文总表。
