...
| P0 | `laptop14 -> rest15` 完整伪标签F1为61.17，最终模型却只有51.64 | 定位并改善伪标签到最终模型的转化效率 | 冻结 DANN 0/0.01/0.03与伪标签高层权重0.65/0.75/0.80矩阵；记录源域/目标伪标签稀疏梯度余弦和范数 | 快速候选相对同次锚点 raw F1至少提高0.50，负面与多三元组F1任一下降不超过0.30；晋级后完整从头验收 |
