...
| 当前代码版本 | `f0e8dc6 Add sentiment prototype contrastive training` |
| 当前分支 | `master` |
...
| 对比学习参数 | `--lambda_sentiment_contrastive 0.05 --sentiment_contrastive_temperature 0.10 --sentiment_contrastive_min_weight 0.65 --sentiment_contrastive_exclude_augment` |
| 对比学习样本 | 1276 句、1812 个三元组：pos=1224、neg=538、neu=50 |
...
| 新模型位置 | `models\final_dann_l0.03_strict_aug150_w020_label_to_text_gen_sentiment_contrastive_l005_ep5` |
...
|---|---|---|---|---|---|---|
| 2026-07-12 | `f0e8dc6 Add sentiment prototype contrastive training` | 情感原型对比学习 | `t5_absa_train.py`、`run_bgca_aste_stage1_pairs.py`、`test_domain_adversarial_train.py` | 从每个三元组的观点词解码器表示计算三类可训练原型损失；支持 batch size=1；排除增强数据；批量脚本复用已有最佳训练文件并使用独立模型/指标标签；后续补充 `--resume_from_checkpoint auto`，训练中断后自动恢复最新检查点 | `runs\bgca_aste_stage1_domain_prompt_text_v1\rest16_to_laptop14\models\final_dann_l0.03_strict_aug150_w020_label_to_text_gen_sentiment_contrastive_l005_ep5` | 待跑；对照 raw F1=46.63、fixed F1=48.98 |
