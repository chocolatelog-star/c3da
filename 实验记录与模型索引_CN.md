...
| 当前新增能力 | 主目录已同步完整双三元组、dynamic strict（动态严格筛选）、严格消融脚本，并新增最终阶段 `final_pseudo_weight`（最终伪标签权重）和 `final_augment_weight`（最终增强权重）消融 |
...
| 2026-07-20 清理状态 | 已删除失败实验的大模型目录：括号式领域前缀、全流程编码器对比、T5 情感向量全流程、三任务混合生成器、语义增强 v2，以及 worktree（工作树）里的 dynamic/complete_dynamic 失败模型；指标、预测和分析文件保留 |
| 2026-07-20 新代码能力 | 主目录 `master`（主分支）同步完整双三元组与动态严格筛选代码；新增 `--final_pseudo_weight` 和 `--final_augment_weight`，用于固定上游后只重建最终训练集并重跑最终 5 轮 |
...
完整双三元组权重消融命令（从 CMD（命令提示符）开始，整行执行）：

```text
cmd /c "J: && cd /d J:\nlp\CD-C3DA && conda activate c3da && python run_bgca_aste_stage1_pairs.py --output_root runs\bgca_aste_stage1_domain_prompt_text_v1 --pairs rest16:laptop14 --reuse_upstream_run_dir runs\bgca_aste_stage1_domain_prompt_text_v1\rest16_to_laptop14 --extractor_model_path J:\nlp\models\t5-base-py --generator_model_path J:\nlp\models\t5-base-py --generator_prompt_style label_to_text --augment_prompt_style masked_mutual --domain_prefix_style text --extractor_epochs 25 --generator_epochs 8 --final_epochs 5 --complete_multi_extra_weight 0.25 --final_pseudo_weight 0.65 --final_augment_weight 0.20 --lambda_sentiment_contrastive 0.01 --sentiment_contrastive_source_only --sentiment_contrastive_class_balanced --learning_rate 0.0003 --eval_batch_size 2 --cuda 0 --seed 1000"
```

增强权重消融命令（从 CMD（命令提示符）开始，整行执行）：

```text
cmd /c "J: && cd /d J:\nlp\CD-C3DA && conda activate c3da && python run_bgca_aste_stage1_pairs.py --output_root runs\bgca_aste_stage1_domain_prompt_text_v1 --pairs rest16:laptop14 --reuse_upstream_run_dir runs\bgca_aste_stage1_domain_prompt_text_v1\rest16_to_laptop14 --extractor_model_path J:\nlp\models\t5-base-py --generator_model_path J:\nlp\models\t5-base-py --generator_prompt_style label_to_text --augment_prompt_style masked_mutual --domain_prefix_style text --extractor_epochs 25 --generator_epochs 8 --final_epochs 5 --complete_multi_extra_weight 0.25 --final_pseudo_weight 0.50 --final_augment_weight 0.30 --lambda_sentiment_contrastive 0.01 --sentiment_contrastive_source_only --sentiment_contrastive_class_balanced --learning_rate 0.0003 --eval_batch_size 2 --cuda 0 --seed 1000"
```

完整双三元组新增样本权重消融命令（从 CMD（命令提示符）开始，整行执行）：

```text
cmd /c "J: && cd /d J:\nlp\CD-C3DA && conda activate c3da && python run_bgca_aste_stage1_pairs.py --output_root runs\bgca_aste_stage1_domain_prompt_text_v1 --pairs rest16:laptop14 --reuse_upstream_run_dir runs\bgca_aste_stage1_domain_prompt_text_v1\rest16_to_laptop14 --extractor_model_path J:\nlp\models\t5-base-py --generator_model_path J:\nlp\models\t5-base-py --generator_prompt_style label_to_text --augment_prompt_style masked_mutual --domain_prefix_style text --extractor_epochs 25 --generator_epochs 8 --final_epochs 5 --complete_multi_extra_weight 0.35 --final_pseudo_weight 0.50 --final_augment_weight 0.20 --lambda_sentiment_contrastive 0.01 --sentiment_contrastive_source_only --sentiment_contrastive_class_balanced --learning_rate 0.0003 --eval_batch_size 2 --cuda 0 --seed 1000"
```
...
|---|---|---|---|---|---|---|
| 2026-07-20 | 本次提交，见 `git log -1` | 完整双三元组主目录同步与权重消融 | `run_bgca_aste_stage1_pairs.py`、`t5_aste_pipeline.py`、`t5_absa_train.py`、`t5_aste_data.py`、严格消融脚本和测试文件 | 将 feature（功能分支）中的完整双三元组、dynamic strict（动态严格筛选）、严格模块消融代码同步到主目录；新增 `--final_pseudo_weight` 和 `--final_augment_weight`，输出 tag（标签）自动带 `pw/aw`，避免覆盖当前最好模型；`build_final_train_from_files` 支持覆盖增强样本权重。 | 复用 `runs\bgca_aste_stage1_domain_prompt_text_v1`，只新增带权重 tag（标签）的最终训练、模型和评估文件 | 代码已改；单元测试 65/65 通过；失败模型目录已清理 |
