...
| 已完成工作 | 当前最好流程已精确复现；六组跨域基线已完成；`rest14 -> laptop14` 观点软过滤、观点契约供给、动态比例与质量分层、两版目标域方面候选发现诊断均已完成归因；`rest15 -> laptop14` 保守多候选最终解码已完成；`laptop14 -> rest15` 完整锚点、三组快速消融和权重0.75完整从头验收均已完成；`rest16 -> laptop14` 权重0.75完整迁移及同上游0.65严格对照均已完成 |
| 最新实验 | `rest16 -> laptop14` 同上游权重0.65严格对照取得 raw P/R/F1（原始精确率/召回率/F1）48.18/39.19/43.22，fixed F1（修正F1）45.51；低于同上游0.75共2.68个百分点，低于48.93保护基线5.71个百分点，也低于BGCA 47.28共4.06个百分点，不晋级 |
| 最新诊断证据 | 0.65运行是 `controlled_stage_reuse`（受控阶段复用）、复用深度1；第1–8阶段均从唯一0.75完整父运行按SHA256硬链接导入，固定385条基础伪标签、457条完整伪标签、150条增强和1463行最终训练集，只重跑第9–10阶段。相同上游下0.75相对0.65增加12个TP、减少17个FP和减少12个FN，证明提高权重对这批上游有正向转化作用，但仍不足以恢复48.93 |
...
| 当前下一步 | 已从正式最佳提交 `d2f2a35` 建立最小分支 `feature/best-upstream-pw075-v1`：从原始数据重跑正式最佳第1–8阶段并逐阶段执行黄金门禁，仅在第9阶段把目标伪标签有效权重从0.65缩放到0.75。若任一上游产物偏离421/494/1499及对应哈希将立即停止；若最终超过48.93，再做候选完整验收 |
| 当前实施计划 | 最小实现提交 `e468446`；只增加目标伪标签权重比例、0.75实验配方和专用入口，不带入后续增强、梯度诊断或阶段复用框架。29项正式入口/复现测试、权重行为测试、语法检查和十阶段干运行均已通过；教师—学生网络和双生成器均未采用 |
...
2026-08-13，实验分支提交 `8704d84` 已实现三步串行的目标域真实句锚定增强：第一步用目标伪标签句做局部方面、观点与中性替换，并经过编辑契约、NLI（自然语言推断）和抽取器精确回抽强过滤；第二步联合约束方面55%–65%、观点25%–35%、中性不超过10%、多三元组不少于50%；第三步增加高/中质量分层权重0.30/0.15与中性0.18/0.10，同时把增强总有效权重限制为30。伪标签有效权重保持0.75，DANN（领域对抗神经网络）保持0.03，仍为单个标签到文本生成器，不采用教师—学生网络。第一步完整从头；第二、三步只能直接复用第一步的准备、抽取器、伪标签和生成器，禁止多代复用。实现通过145项测试、语法检查和命令图干运行；GPU（图形处理器）实验结果待本轮串行运行完成后补录。
...
严格对照已于2026-08-12完成。复用来源记录确认第1–8阶段全部来自唯一父运行，导入方式为硬链接，抽取器、生成器、伪标签、增强和最终训练集SHA256均与父运行一致；第9–10阶段退出码均为0。0.65取得 raw P/R/F1 48.18/39.19/43.22，fixed F1 45.51，TP/FP/FN为212/228/329。相同上游的0.75为51.49/41.40/45.90，TP/FP/FN为224/211/317；因此0.75让精确率提高3.31个百分点、召回率提高2.21个百分点、F1提高2.68个百分点，并增加12个TP、减少17个FP和12个FN。

分项结论一致：0.75相对0.65将正面F1从45.33提高到49.42，负面F1从48.76轻微降到48.46，中性两者都为0；单三元组F1从46.31提高到49.87，多三元组F1从41.16提高到43.27。提高伪标签权重主要改善正面、整体精确率和多三元组输出，没有恢复中性。梯度诊断中，源域/伪标签平均梯度范数比从0.65的1.55降至0.75的1.34，负余弦比例从47.52%变为50.00%，说明更高权重确实增强了目标伪标签信号；约一半梯度对仍存在冲突，但本次证据不支持把冲突比例本身作为性能下降的直接原因。

最终因果结论是：对于该次385/457条较弱上游轨迹，0.65欠利用目标伪标签，0.75明显更好；然而0.75完整运行仍只有45.90，低于48.93共3.03个百分点。因此0.75不是可迁移的新主线，0.65也不能“还原”出48.93；48.93与本次完整运行的主要差异仍来自第1–8阶段的抽取器随机轨迹、少37条基础伪标签、少37条完整伪标签以及增强内容变化。该方向停止继续扫描权重，保留48.93保护基线，后续优先重构增强质量和覆盖。

本次必须严格区分“最佳流程复现”和“改造分支完整实验”。虽然实验分支以主分支 `d2f2a35` 为共同祖先，但实际运行提交是累计加入增强契约、调度器、训练诊断和阶段复用等改动后的 `606e2ec`，并非主分支或正式验收提交 `558e4de`；运行配方也是实验配方，不是带完整黄金模型、数据和预测SHA256门禁的 `rest16_to_laptop14_best_v1.json`。实验配方沿用 `historical_seed_only`（仅历史种子）随机模式，只固定随机种子，不能保证代码路径变化后随机数消费顺序仍与历史完全相同；又因未携带黄金阶段门禁，抽取器首次偏离黄金哈希、基础伪标签从421降至385时没有立即停止。故45.90是“改造分支完整从头实验失败”，不是“主分支48.93再次复现失败”。后续不得再把实验分支的 `full_from_scratch`（完整从头）称为最佳流程复现；凡声称复现48.93，必须使用主分支/正式验收提交、正式最佳配方和逐阶段黄金门禁。

2026-08-12经用户明确许可，已永久删除0.75完整运行的6个 `checkpoint-*`（训练检查点）和失败最终 `best`（最佳）模型，以及0.65受控对照的2个训练检查点和失败最终最佳模型，共10个目录，释放约21.631 GiB（吉比字节）。两次运行的指标、预测、日志、命令、清单、梯度诊断、复用来源记录，以及0.75运行中用于审计的抽取器和生成器最佳模型均保留。两次失败训练均不能再从第9阶段中间步恢复或直接重新评估已删除的最终模型；48.93保护基线和独立验收现场未触碰。
