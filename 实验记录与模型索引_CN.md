...
| 已完成工作 | 当前最好流程已精确复现；六组跨域基线已完成；`rest14 -> laptop14` 观点软过滤250条、观点契约供给150条、动态比例与质量分层实验均已从头运行并完成归因 |
| 最新实验 | 动态比例与质量分层 raw/fixed F1（原始/修正F1）=**50.25/53.82**，低于同方向旧契约150条的52.60/55.09；未达到第一门槛，不扩展到其他方向 |
...
验证结果：62项相关回归测试全部通过；历史配方阶段图保持不变；新配方干运行严格停在第5阶段；RTX 3070 两行显卡冒烟测试通过，两个 T5 模型按顺序加载和释放，候选解码与重构评分均按第1、2行写入断点，重构损失均为有限值。正式诊断尚未运行，因此目前没有新实验指标或可删除模型。
