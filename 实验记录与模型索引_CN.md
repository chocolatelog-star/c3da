...
| 当前工作 | 已完成当前代码原生十阶段入口：不调用历史工作树，不读取其他运行目录；每阶段显式声明输入并校验输入/输出哈希 |
| 当前候选分支 | `feature/native-best-reproduction-v1`；关键提交 `afc0d3d`、`303225a`、`00427d2`、`38cb6f3`、`1a65631`、`1589b2f`、`15bd055` |
| `master` 状态 | `6815bb7`，保持当前已验证版本；原生候选通过完整 GPU 验收前不合并 |
| 当前首次待验证偏差 | 首次 GPU 验收在抽取器 16/1325 step 因 Windows GBK 控制台无法输出替代字符而中断；模型/CUDA 正常，已增加安全输出修复，等待新提交重跑 |
| 当前主要模型短板 | neutral（中性）召回接近 0；多三元组召回仍低；历史最佳 recall（召回率）仅 42.14 |
| 当前下一步 | 全项目测试、候选分支推送、独立当前代码检出中的 RTX 3070 完整十阶段验收 |
...
| 当前代码原生候选 | 计划是；禁止读取旧运行产物 | 待跑 | 待跑 | 待跑 | 待跑 | 待跑 | GPU 验收前不进 `master` |
...
| 最终训练集 | 1499 | `4876753D495A284FCAA454004CB441099421415B0DEFC353C5EDDA2E2FF36A88` |
...
| 当前代码原生迁移 | `feature/native-best-reproduction-v1`；`afc0d3d..15bd055` | 配方 `rest16_to_laptop14_best_v1` | 待跑 | 待跑 | 已完成来源隔离、命令归档、黄金校验、增强兼容和试运行 | 候选代码保留 |
...
|---|---|---|---|---|
| P0 | 当前代码原生流程尚未经过完整 GPU 验收 | 证明不依赖历史工作树也能精确复现 | 在独立当前代码检出中从头运行十阶段并逐阶段核对哈希 | 八项关键哈希、TP/FP/FN、raw 48.93、fixed 50.21 全部一致 |
...
当前代码原生完整 GPU 命令将在候选代码完成所有测试并放入独立、干净、非历史工作树的当前代码检出后执行。正式命令不使用 `-AllowDirtyDiagnostic`；断电后重复同一命令和同一 `RunId` 恢复。
