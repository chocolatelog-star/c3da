...
| 已完成工作 | 当前最好流程已精确复现；六组跨域基线已完成；`rest14 -> laptop14` 观点软过滤、观点契约供给、动态比例与质量分层、两版目标域方面候选发现诊断均已完成归因；`rest15 -> laptop14` 保守多候选最终解码已完成；`laptop14 -> rest15` 完整锚点、DANN（领域对抗神经网络）=0、伪标签高层权重0.75和0.80四个最终训练转化实验均已完成 |
| 最新实验 | `laptop14 -> rest15` 权重0.75快速消融取得当前组内最好 raw P/R/F1（原始精确率/召回率/F1）56.98/51.34/54.01，fixed F1（修正F1）55.53；比同次锚点提高2.81，负面F1提高3.79，多三元组F1提高4.82，但仍低于 BGCA 58.95共4.94个百分点，尚不是正式完整验收结果 |
| 最新诊断证据 | 权重0.75完整从头验收的第1–8阶段全部成功，第9阶段在首个训练步前由 `nvcuda64.dll`（NVIDIA CUDA驱动库）以异常码 `0xC0000409` 原生崩溃。抽取器、生成器、增强及最终训练语义哈希与成功快速消融完全一致，提交间没有训练代码变化，因此当前定性为环境/驱动中断，不是模型结果失败 |
...
| 当前下一步 | 权重0.75完整从头验收停在第9阶段，等待用同一运行标识恢复；恢复入口会校验并跳过已经完成的第1–8阶段，只从尚无检查点的最终训练重新开始。当前不改代码、不改参数，也不把该驱动崩溃计为模型失败 |
...
### 2.6 `laptop14 -> rest15` 最终训练转化诊断 v1（快速消融完成，权重0.75晋级）
...
权重0.75相对锚点增加26个TP（真阳性）、25个FP（假阳性）并减少26个FN（假阴性），核心收益来自召回提高5.36个百分点；负面F1提高3.79，多三元组F1提高4.82。逐句比较有40行改善、28行退化、254行不变。权重0.80比0.75少8个TP、少15个FP，表现重新偏保守；说明目标伪标签权重不是越高越好，0.75是当前数据上的更好平衡点。
...
最终开发集最佳损失也支持相同排序：锚点0.5885（第3轮）、DANN=0为0.5740（第4轮）、权重0.75为0.5604（第4轮）、权重0.80为0.5789（第4轮）。配对测试集自助抽样中，权重0.75相对锚点的F1提升95%区间约为-0.46到+6.23个百分点，单个随机种子仍不足以形成正式结论，因此必须完整从头复现。

权重0.75把完整伪标签61.17到最终模型的差距从锚点约9.96缩小到约7.16，但最终54.01仍低于BGCA 58.95共4.94个百分点，且所有版本中性F1均为0。数据增强文件在四组中完全相同，增强绝对有效权重始终只有29.80，在锚点总权重中占2.54%，在权重0.75版本中进一步降至2.46%。所以这轮证明了提高目标伪标签信号有效，却没有解决增强弱、目标方面覆盖和中性缺失问题。

磁盘上没有 DANN=0.01运行目录。由于DANN=0虽然提高总体F1但未通过负面保护门槛，按预先冻结的条件式策略不再补跑0.01是合理的。下一步先为权重0.75建立完整从头验收配方；复现后再把主线转向以目标域真实句为主、源域金标迁移为辅的混合增强重构。
...
| P0 | `laptop14 -> rest15` 完整伪标签F1为61.17，锚点最终模型只有51.21 | 提高伪标签到最终模型的转化效率 | 梯度诊断和冻结消融已完成；权重0.75取得54.01并通过全部保护门槛，DANN 0与权重0.80不晋级 | 为权重0.75建立新运行标识并完整从原始数据运行第1–10阶段；只有从头复现后才作为后续增强基线 |
