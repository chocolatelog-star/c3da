...
def resolve_model_path(model_root, preferred_path, hf_name):
    if preferred_path:
        return preferred_path
    local_name = hf_name.replace('/', '-')
    local_path = os.path.join(model_root, local_name)
    return local_path if os.path.isdir(local_path) else hf_name

def resolve_cross_domain_opt(opt, dataset_files, vocab_dirs):
    source = opt.source_dataset or opt.dataset
    target = opt.target_dataset or opt.dataset
    if source not in dataset_files:
        raise ValueError('unknown source_dataset: {}'.format(source))
    if target not in dataset_files:
        raise ValueError('unknown target_dataset: {}'.format(target))

    opt.source_dataset = source
    opt.target_dataset = target
    opt.dataset = source
    opt.exp_name = '{}_to_{}'.format(source, target) if source != target else source
    opt.dataset_file = {
        'train': dataset_files[source]['train'],
        'test': dataset_files[target]['test'],
        'aug': dataset_files[source]['aug'],
    }
    opt.vocab_dir = vocab_dirs[source]
    return opt

def split_source_train_dev(dataset, dev_ratio, seed):
    if dev_ratio <= 0 or dev_ratio >= 1:
        raise ValueError('dev_ratio must be in (0, 1), got {}'.format(dev_ratio))
    indices = np.arange(len(dataset))
    rng = np.random.RandomState(seed)
    rng.shuffle(indices)
    dev_size = max(1, int(len(indices) * dev_ratio))
    dev_indices = indices[:dev_size].tolist()
    train_indices = indices[dev_size:].tolist()
...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        source_set = ABSAData(opt.dataset_file['train'], tokenizer, opt=opt)
        train_indices, dev_indices = split_source_train_dev(source_set, opt.dev_ratio, opt.seed)
        trainset = Subset(source_set, train_indices)
        devset = Subset(source_set, dev_indices)
        target_testset = ABSAData(opt.dataset_file['test'], tokenizer, opt=opt)
        augset = None
        if opt.withAugment:
            full_augset = ABSAAugmentData(opt.dataset_file['aug'], tokenizer)
            augset = Subset(full_augset, train_indices)

        self.train_dataloader = DataLoader(dataset=trainset, batch_size=opt.batch_size, shuffle=False)
        self.dev_dataloader = DataLoader(dataset=devset, batch_size=opt.batch_size)
        self.target_test_dataloader = DataLoader(dataset=target_testset, batch_size=opt.batch_size)
        if opt.withAugment:
            # Augmented samples are variable-length tensors, so the default collate cannot stack them.
            self.aug_dataloader = DataLoader(dataset=augset, batch_size=opt.batch_size, collate_fn=collate_fn, shuffle=False)

        self.closs = ContrastiveLoss(batch_size=opt.batch_size * opt.k, margin=opt.margin)
    ...
    def _train(self, criterion, optimizer, best_source_dev_f1_overall=0):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            aug_iter = iter(self.aug_dataloader) if self.opt.withAugment else None

            for _ in range(self.train_dataloader.__len__()):
                vanilla_sample = next(vanilla_iter)
                global_step += 1
                ...
                if self.opt.withAugment:
                    ...
                    for k in range(inputs_copy[0].shape[0]):
                        start = int(torch.sum(inputs_copy[2][k]).item())
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    dev_acc, dev_f1 = self._evaluate(self.dev_dataloader)
                    if dev_f1 > max_f1:
                        max_f1 = dev_f1
                        if dev_f1 > best_source_dev_f1_overall:
                            if not os.path.exists('./C3DA/state_dict'):
                                os.makedirs('./C3DA/state_dict')
                            model_path = './C3DA/state_dict/{}_{}_source_dev_f1_{:.4f}'.format(self.opt.model_name, self.opt.exp_name, dev_f1)
                            self.best_model = copy.deepcopy(self.model)
                    ...
                    if self.opt.withAugment:
                        if self.opt.withCL:
                            logger.info('loss: {:.4f}, VanillaLoss:{:.4f}, AugLoss:{:.4f}, CLLoss:{:.4f}, train_acc: {:.4f}, source_dev_acc: {:.4f}, source_dev_f1: {:.4f}'
                                        .format(loss.item(), loss_van.item(), loss_aug.item(), loss_cl.item(), train_acc, dev_acc, dev_f1))
                        else:
                            logger.info('loss: {:.4f}, VanillaLoss:{:.4f}, AugLoss:{:.4f}, train_acc: {:.4f}, source_dev_acc: {:.4f}, source_dev_f1: {:.4f}'
                                .format(loss.item(), loss_van.item(), loss_aug.item(), train_acc, dev_acc, dev_f1))
                    else:
                        logger.info('loss: {:.4f}, VanillaLoss:{:.4f}, train_acc: {:.4f}, source_dev_acc: {:.4f}, source_dev_f1: {:.4f}'.format(loss.item(), loss_van.item(), train_acc, dev_acc, dev_f1))
    ...
    def _test(self):
        ...
        self.model.eval()
        test_report, test_confusion, acc, f1 = self._evaluate(self.target_test_dataloader, show_results=True)
        logger.info("Final Target-Domain Precision, Recall and F1-Score...")
        logger.info("target_test_acc:{}".format(acc))
        logger.info("target_test_f1:{}".format(f1))
        logger.info(test_report)
        logger.info("Final Target-Domain Confusion Matrix...")
    ...
    def run(self):
        ...
        best_source_dev_acc_overall = 0
        best_source_dev_f1_overall = 0
        if 'bert' not in self.opt.model_name:
            self._reset_params()
        best_source_dev_acc, best_source_dev_f1, model_path = self._train(criterion, optimizer, best_source_dev_f1_overall)
        logger.info('best_source_dev_acc: {0}, best_source_dev_f1: {1}'.format(best_source_dev_acc, best_source_dev_f1))
        best_source_dev_acc_overall = max(best_source_dev_acc, best_source_dev_acc_overall)
        best_source_dev_f1_overall = max(best_source_dev_f1, best_source_dev_f1_overall)
        if not model_path:
            if not os.path.exists('./C3DA/state_dict'):
                os.makedirs('./C3DA/state_dict')
            model_path = './C3DA/state_dict/{}_{}_final'.format(self.opt.model_name, self.opt.exp_name)
        torch.save(self.best_model.state_dict(), model_path)
        logger.info('>> saved: {}'.format(model_path))
        logger.info('#' * 60)
        logger.info('best_source_dev_acc_overall:{}'.format(best_source_dev_acc_overall))
        logger.info('best_source_dev_f1_overall:{}'.format(best_source_dev_f1_overall))
        self._test()
...
def main():
    ...
    parser.add_argument('--source_dataset', default='', type=str, help='source domain for cross-domain training')
    parser.add_argument('--target_dataset', default='', type=str, help='target domain for cross-domain evaluation')
    ...
    parser.add_argument('--dev_ratio', default=0.1, type=float, help='ratio of source training data used for source-domain checkpoint selection')
    ...
    parser.add_argument('--roberta_path', default='', type=str)
    ...
    opt = resolve_cross_domain_opt(opt, dataset_files, vocab_dirs)
    opt.model_class = model_classes[opt.model_name]
    opt.inputs_cols = input_colses[opt.model_name]
    opt.initializer = initializers[opt.initializer]
    opt.optimizer = optimizers[opt.optimizer]
    model_paths = {
        'bert': resolve_model_path(opt.model_root, opt.bert_path, model_classes['bert']),
        'roberta': resolve_model_path(opt.model_root, opt.roberta_path, model_classes['roberta']),
        'bartencoder': resolve_model_path(opt.model_root, '', model_classes['bartencoder']),
    }
    opt.pretrained_bert_name = model_paths[opt.model_name]
    ...
    log_file = '{}-{}-s{}-{}.log'.format(opt.model_name, opt.exp_name, opt.seed, strftime("%Y-%m-%d_%H-%M-%S", localtime()))
