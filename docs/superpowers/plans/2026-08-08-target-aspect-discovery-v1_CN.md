...
**v2 实施状态（2026-08-09）：** 已在 `feature/source-calibrated-aspect-discovery-v2` 完成源域独立联合门槛，提交为 `b16fab0`。源域开发集采用与目标域同构的4路候选解码和生成器评分，只从预先固定的支持度与损失分位点网格中选择门槛；必须满足源域精确率至少65%且至少保留30条，否则阶段停止。门槛冻结后才应用于目标域，目标隐藏金标仍只作选择完成后的审计。新配方为 `rest14_to_laptop14_target_aspect_discovery_source_calibrated_v2.json`，保持单生成器且不加入教师—学生网络；62项相关回归测试通过，正式单种子诊断待用户运行。
