...
- 当前4090约24 GB显存默认并行2个独立实验；只有资源评估证明单任务峰值显存不超过约7 GB、三任务预计总峰值后仍预留至少3 GB，且CPU、内存、磁盘与数据加载稳定时才并行3个。
...
- 开启GPU模式前必须具备：已推送提交、干净工作区、冻结单行命令、唯一输出目录、恢复入口和资源预算核验。从新正式目录直接运行。
